...
QUERIES   = "full_clusters_temp.fasta"
TSVS      = ["full_big_members.tsv", "full_medium_members.tsv", "full_small_members.tsv"]
SUBSAMPLE = 40
EVALUE    = "1e-20"
DUST      = "no"
...
CRE = re.compile(r"^(BIG|MEDIUM|SMALL|SINGLE)_.*?_size(\d+)_(.+)$")
...
def main():
    ...
    print("-"*74)
    for cls in ("BIG", "MEDIUM", "SMALL", "SINGLE"):
        s = st.get(cls)
